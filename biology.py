...
def get_vista_top_20_bodies():
    cursor = mysql_conn.cursor(pymysql.cursors.Cursor)
    sql = """
   select data.system,data.body,cast(sum(reward) as char) as value,cast(count(distinct data.entryid) as char) as species from (
select distinct cr.system,cr.body,cr.entryid from codexreport cr  where entryid in (
	select entryid from codex_name_ref where hud_category = 'Biology'
)
and body is not null) data
join  (
	SELECT
		entryid,cast(SUBSTRING_INDEX( GROUP_CONCAT(reward ORDER BY created_at DESC), ',', 1) as SIGNED) as reward
      from (
       select
       entryid,
       reward,created_at
   FROM organic_sales os
      ...
      group by entryid) rewards on rewards.entryid = data.entryid
group by data.system,data.body
...
def get_vista_top_20_systems():
    cursor = mysql_conn.cursor(pymysql.cursors.Cursor)
    sql = """
   select data.system,cast(sum(reward) as char) as value,cast(count(distinct data.entryid) as char) as species from (
select distinct cr.system,cr.body,cr.entryid from codexreport cr  where entryid in (
	select entryid from codex_name_ref where hud_category = 'Biology'
)
and body is not null) data
...
def populate_bio_systems(system, body, journal_id, journal_name, frontier_id, x, y, z, site_type):
    ...
    have_journal = journal_exists(
        bio_systems[system]["body"][body]["journal"], journal_id)
    no_site = (site_id == "None")
    have_site = (site_id != "None")
    journal_none = bio_systems.get(system).get("body").get(
        body).get("journal").get(journal_id).get("None")

    if have_site and journal_none:
        del bio_systems[system]["body"][body]["journal"][journal_id]["None"]

    if have_journal:
        bio_systems[system]["body"][body]["journal"][journal_id][site_id] = {
            "type": site_type, "name": journal_name}
    else:
        bio_systems[system]["body"][body]["journal"][journal_id] = {}
        bio_systems[system]["body"][body]["journal"][journal_id][site_id] = {
            "type": site_type, "name": journal_name}
...
def get_sites(site):
    ...
    for row in capi:
        ...
        body = str(row.get("body").get("bodyName").replace(
        ...
        populate_bio_systems(system, body, journal_id,
                             journal_name, frontier_id, x, y, z, site)
...
def summarise_biosystems():
    for system, systemdata in bio_systems.items():
        ...
        for body, bodydata in systemdata.get("body").items():
            bio_systems[system]["body"][body]["codex_sites"] = count_sites(
                bodydata)
            bio_systems[system]["body"][body]["type_counts"] = get_type_array(
                bodydata)
...
def get_signal_cells():
    for signal in bio_signals:
        ...
        has_bodies = (has_system and bio_systems.get(
            signal.get("system")).get("body"))
        has_body = (has_bodies and bio_systems.get(
            signal.get("system")).get("body").get(signal.get("body")))
        system = signal.get('system')
        if has_body:
            #print(f"XXX BODY EXISTS {system}")
            bio_systems[system]["body"][signal.get(
                "body")]["signals"] = signal.get("count")
            bio_systems[system]["body"][signal.get("body")]["has_body"] = True
        elif has_bodies:
            #print(f"XXX SOMEBODY EXISTS {system}")
            bio_systems[system]["body"][signal.get("body")] = {
                "has_bodies": True, "signals": signal.get("count"), "codex_sites": 0}
        elif has_system:
            #print(f"XXX SYSTEM EXISTS {system}")
            bio_systems[signal.get("system")]["body"] = {}
            bio_systems[signal.get("system")]["body"][signal.get("body")] = {
                "has_system": True, "signals": signal.get("count"), "codex_sites": 0}
        else:
            #print(f"XXX ORPHAN {system}")
            bio_systems[signal.get("system")] = {}
            bio_systems[signal.get("system")]["body"] = {}
            bio_systems[signal.get("system")]["body"][signal.get("body")] = {
                "orphan": True, "signals": signal.get("count"), "codex_sites": 0}
...
bodies = get_vista_top_20_bodies()
write_sheet(SHEETID, 'Vista Genomics!G3:Z', bodies)
systems = get_vista_top_20_systems()
write_sheet(SHEETID, 'Vista Genomics!G26:Z', systems)
